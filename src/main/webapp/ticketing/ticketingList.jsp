	...
	<form name="formm" method="post">
		<div id="t_listBox">
			...
			<c:forEach items="${ticketList}" var="ticket_productVO">
				...
				<!-- 오늘날짜 -->
				<fmt:parseDate var="sdate" value="${ticket_productVO.sdate}"
					pattern="yyyy-MM-dd HH:mm:ss" />
				<fmt:parseDate var="edate" value="${ticket_productVO.edate}"
					pattern="yyyy-MM-dd HH:mm:ss" />
				...
				<table id="t_list">
					<tr>
						<th rowspan="2"><img
							src="ticket_image/${ticket_productVO.image}" width="190"></th>
					</tr>
					<tr>
						<td>
							<table>
								...
								<tr>
									<td id="t_line">일정</td>
									<td><fmt:formatDate pattern="yyyy/MM/dd" value="${sdate}"
											type="date" /> ~ <fmt:formatDate pattern="yyyy/MM/dd"
											value="${edate}" type="date" /></td>
								</tr>
								<tr>
									<td id="t_line">장소</td>
								...
								<tr>
									...
									<td>공연시간 : ${ticket_productVO.showtime}<br> 시작시간 :
										${ticket_productVO.daytime}
									</td>
								</tr>
								<tr>
									<td id="t_line">가격</td>
									<td>일 반 : ${ticket_productVO.price1} 원<br> 어린이 :
										${ticket_productVO.price2} 원
									</td>
								</tr>
								<tr>
											...
											<c:when test="${ today < enddate }">
												<c:choose>
													<c:when test="${ today < stdate }">
														<a
															href="ticket.do?command=ticketshow&tpseq=${ticket_productVO.tpseq}"
													...
													<c:otherwise>
														<a
															...
															class="btnCancel">상세정보</a>
														<a
															href="ticket.do?command=ticketingForm&tpseq=${ticket_productVO.tpseq}"
